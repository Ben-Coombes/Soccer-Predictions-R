...
counter <- 0
for (p in 1:10) {
  ahurl <- "https://www.soccerstats.com/homeaway.asp?league=france2#"
  H <- getURL(ahurl)
  ...
  matchesurl <-
    "https://www.soccerstats.com/latest.asp?league=france2"
  matches <- getURL(matchesurl)
  matches <- readHTMLTable(matches, stringsAsFactors = F,  which = 9)
  matches$V1 <- NULL
  matches$V3 <- NULL
  matches$V4 <- NULL
  matches$V5 <- NULL
