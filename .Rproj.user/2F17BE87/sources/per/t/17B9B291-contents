...
counter <- 0
for (p in 1:10) {
  ahurl <- "https://www.soccerstats.com/homeaway.asp?league=usa#"
  H <- getURL(ahurl)
  H <- readHTMLTable(H, stringsAsFactors = F, which = 9)
  ...
  A <- getURL(ahurl)
  A <- readHTMLTable(A, stringsAsFactors = F, which = 10)
  ...
  matchesurl <-
    "https://www.soccerstats.com/latest.asp?league=usa"
  matches <- getURL(matchesurl)
  matches <- readHTMLTable(matches, stringsAsFactors = F,  which = 49)
  matches$V1 <- NULL
  matches$V3 <- NULL
  matches$V4 <- NULL
  matches$V5 <- NULL
