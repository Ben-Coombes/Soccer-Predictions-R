...
counter <- 0
for (p in 1:10) {
  ahurl <- "http://www.soccerstats.com/homeaway.asp?league=england3"
  ...
  matchesurl <-
    "http://www.soccerstats.com/latest.asp?league=england3#"
...
LeagueOne <- do.call('rbind', LeagueOne)
rownames(LeagueOne) <- NULL
LeagueOne["Decimal"] <- round(100 / LeagueOne$Odds, 2)
