...
counter <- 0
for (p in 1:12) {
  ahurl <- "http://www.soccerstats.com/homeaway.asp?league=england4"
  ...
  matchesurl <-
    "http://www.soccerstats.com/latest.asp?league=england4#"
  ...
  assign(paste("PROB", p, sep = ""),
         subset(prediction, prediction$Odds >= 80))
  
}
LeagueTwo <-
  mget(paste('PROB', 1:counter, sep = ""), envir = parent.frame())
LeagueTwo <- do.call('rbind', LeagueTwo)
rownames(LeagueTwo) <- NULL
LeagueTwo["Decimal"] <- round(100 / LeagueTwo$Odds, 2)
