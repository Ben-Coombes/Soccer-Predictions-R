...
counter <- 0
for (p in 1:10) {
  ahurl <- "http://www.soccerstats.com/homeaway.asp?league=norway"
  H_A <- readHTMLTable(ahurl, which = 8)
  ...
  matchesurl <-
    "http://www.soccerstats.com/latest.asp?league=norway#"
  matches <- readHTMLTable(matchesurl, which = 10)
  matches$V1 <- NULL
  matches$V3 <- NULL
  matches$V4 <- NULL
...
Eliteserien <- do.call('rbind', Eliteserien)
rownames(Eliteserien) <- NULL
Eliteserien["Decimal"] <- round(100 / Eliteserien$Odds, 2)
